...
pink::ReadStatus PikaBinlogParser::ScrubReadBuffer(const char* rbuf, int len, int* processed_len, int* scrubed_len, BinlogHeader* binlog_header, PortBinlogItem* binlog_item) {
...
pink::ReadStatus PikaBinlogParser::BinlogParseHeader(const char* rbuf, int len, int* processed_len, int* scrubed_len, BinlogHeader* binlog_header, PortBinlogItem* binlog_item) {
 if (len + half_binlog_buf_.size() < HEADER_LEN) {
    ...
    *processed_len += len;
    ...
    if (!PortBinlogTransverter::BinlogHeaderDecode(PortTypeFirst, half_binlog_buf_ + buffer_str, &binlog_header_)) {
...
pink::ReadStatus PikaBinlogParser::BinlogParseDecode(const char* rbuf, int len, int* processed_len, int* scrubed_len, BinlogHeader* binlog_header, PortBinlogItem* binlog_item) {
  if (binlog_header_.header_type_ == kTypeAuth) {
    ...
    *processed_len += len;
    ...
    std::string buffer_str(rbuf, buf_len);
    if (!PortBinlogTransverter::BinlogItemWithoutContentDecode(PortTypeFirst, half_binlog_buf_ + buffer_str, &binlog_item_)) {
      return pink::kParseError;
    }
...
pink::ReadStatus PikaBinlogParser::BinlogParseContent(const char* rbuf, int len, int* processed_len, int* scrubed_len, BinlogHeader* binlog_header, PortBinlogItem* binlog_item) {
